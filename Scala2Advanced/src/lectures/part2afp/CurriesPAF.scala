...
object CurriesPAF extends App {
  ...
  val add7_5 = simpleAddMethod(7, _: Int) // alternative syntax turning methods into function values
  // y => simpleAddMethod(7, y)

  // underscores are powerful
  def concatenator(a: String, b: String, c: String) = a + b + c
  val insertName = concatenator("Hello, ", _: String, ", how are you?") // x: String => concatenator("Hello, ", x, "how are you?")
  println(insertName("Mauricio"))

  val fillInTheBlanks = concatenator("Hello, ", _: String, _: String) // (x, y) => concatenator("Hello, ", x, y)
  println(fillInTheBlanks("Mauricio", " Scala is awesome"))
   ...
   */
  def curriedFormater(s: String)(number: Double): String = s.format(number)
  val numbers = List(Math.PI, Math.E, 1, 9.8, 1.3e-12)

  val simpleFormat = curriedFormater("%4.2f") _
  val seriousFormat = curriedFormater("%8.6f") _
  val preciseFormat = curriedFormater("%14.12f") _

  println(numbers.map(simpleFormat))
  println(numbers.map(seriousFormat))
  println(numbers.map(preciseFormat))
  println(numbers.map(curriedFormater("%14.12f")))
