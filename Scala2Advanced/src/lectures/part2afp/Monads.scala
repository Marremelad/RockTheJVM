...
object Monads extends App {
  ...
  val attemptDivideByZero = (x: Int) => Attempt(x / 0)

  println(Attempt(1).flatMap[Int](x => Attempt(x / 0)))
  println(attemptDivideByZero(1))

  val attempt = Attempt {
    throw new RuntimeException("My own monad, yes!")
  }

  val attempt2 = Attempt(throw new RuntimeException("Some error!"))
